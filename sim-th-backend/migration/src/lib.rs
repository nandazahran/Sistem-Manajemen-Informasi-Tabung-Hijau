pub use sea_orm_migration::prelude::*;

// 1. Panggil file migrasi wilayahmu (sesuaikan dengan nama file aslinya tanpa ekstensi .rs)
// Biasanya formatnya: mod mYYYYMMDD_HHMMSS_create_tabel_wilayah;
mod m20260502_013829_create_tabel_wilayah;
mod m20260502_014459_create_tabel_kategori_sampah;
mod m20260502_015120_create_tabel_user;
mod m20260502_022617_create_tabel_transaksi_sampah;
mod m20260502_023012_create_tabel_tabungan_sampah;
mod m20260504_110625_tambah_kolom_totp_user;
mod m20260504_131630_tambah_kolom_otp_reset;
mod m20260507_070235_create_kontak_table;
mod m20260527_150521_create_tabel_rekening_wilayah;
mod m20260531_135247_create_tabel_notifikasi;
mod m20260610_144218_create_riwayat_harga_table;
mod m20260610_150546_create_riwayat_penarikan_table;

pub struct Migrator;

#[async_trait::async_trait]
impl MigratorTrait for Migrator {
    fn migrations() -> Vec<Box<dyn MigrationTrait>> {
        vec![
            Box::new(m20260502_013829_create_tabel_wilayah::Migration),
            Box::new(m20260502_014459_create_tabel_kategori_sampah::Migration),
            Box::new(m20260502_015120_create_tabel_user::Migration),
            Box::new(m20260502_022617_create_tabel_transaksi_sampah::Migration),
            Box::new(m20260502_023012_create_tabel_tabungan_sampah::Migration),
            Box::new(m20260504_110625_tambah_kolom_totp_user::Migration),
            Box::new(m20260504_131630_tambah_kolom_otp_reset::Migration),
            Box::new(m20260507_070235_create_kontak_table::Migration),
            Box::new(m20260527_150521_create_tabel_rekening_wilayah::Migration),
            Box::new(m20260531_135247_create_tabel_notifikasi::Migration),
            Box::new(m20260610_144218_create_riwayat_harga_table::Migration),
            Box::new(m20260610_150546_create_riwayat_penarikan_table::Migration),
        ]
    }
}
