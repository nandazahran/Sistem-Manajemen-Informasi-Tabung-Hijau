							...
							{showNotifPopup && (
								<div className="absolute right-0 mt-4 w-80 bg-white rounded-3xl shadow-[0_15px_40px_rgba(0,0,0,0.12)] border border-gray-100 overflow-hidden z-50 animate-fade-in">
									<div className="p-5 border-b border-gray-100">
										<h3 className="font-extrabold text-[#0B4D1E] text-lg tracking-tight">
											Notifikasi
										</h3>
									...
									<div className="max-h-[300px] overflow-y-auto">
										{dataNotif.length > 0 ? (
											dataNotif.map((n, i) => (
												<div
													key={i}
													className="p-5 border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer flex gap-4 items-start"
												>
													<div className="mt-1.5 flex-shrink-0">
														<div
															className={`w-2.5 h-2.5 rounded-full ${!n.isRead ? "bg-[#F4A300]" : "bg-transparent"}`}
														></div>
													</div>
													<div>
														<p className="text-sm font-extrabold text-[#0B4D1E]">
															{n.judul || "Pemberitahuan Baru"}
														</p>
														<p className="text-xs text-gray-500 mt-1 line-clamp-2 leading-relaxed">
															{n.pesan || n.deskripsi}
														</p>
														<p className="text-[10px] text-gray-400 mt-2 font-bold">
															{formatWaktuNotif(n.waktu || n.created_at)}
														</p>
													</div>
												</div>
											))
