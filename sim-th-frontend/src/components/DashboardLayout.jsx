...
function DashboardLayout({ children }) {
  ...
  return (
    // overflow-x-hidden DIHAPUS DARI SINI BIAR STICKY HEADER JALAN
    <div className="min-h-screen bg-[#F5EFE6] flex font-sans">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.name} to={item.path} className={`flex items-center gap-4 px-6 py-3.5 rounded-2xl transition-all duration-300 font-medium ${isActive ? 'bg-[#F4A300] text-white shadow-lg' : 'text-green-100 hover:bg-[#083a16] hover:text-white'}`}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d={item.icon} /></svg>
                {item.name}
              </Link>
            );
          })}
      ...
      <div className={`flex-1 flex flex-col transition-all duration-500 ease-in-out ${isSidebarOpen ? 'ml-72' : 'ml-0'}`}>
        ...
        {/* HEADER BACKGROUND PUTIH (bg-white) & TAMBAH SHADOW TIPIS BIAR CAKEP */}
        <header className="h-24 bg-white flex items-center justify-between px-10 sticky top-0 z-30 shadow-sm border-b border-gray-100">
          <div className="flex items-center gap-6 flex-1">
            ...
            <div className="relative w-full max-w-2xl">
              ...
              <input type="text" placeholder="Cari transaksi, kategori, laporan, atau tanggal..." className="w-full bg-[#F5EFE6] px-14 py-4 rounded-full focus:outline-none focus:ring-2 focus:ring-[#0B4D1E] transition-all text-sm font-medium placeholder-gray-500" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4 cursor-pointer hover:opacity-80 transition-opacity">
              <div className="text-right">
                <p className="font-extrabold text-[#0B4D1E] text-sm">BEM FATETA</p>
                <p className="text-xs text-[#F4A300] font-bold bg-[#F4A300]/10 inline-block px-3 py-1 rounded-full mt-1">BEM Wilayah</p>
              </div>
              <div className="w-12 h-12 bg-linear-to-br from-[#8FA57A] to-[#0B4D1E] rounded-full flex items-center justify-center text-white shadow-md">
        ...
        <main className="flex-1 px-10 pb-4">
          {children}
        </main>
