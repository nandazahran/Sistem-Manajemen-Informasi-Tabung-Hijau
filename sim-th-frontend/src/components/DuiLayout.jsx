...
function DuiLayout({ children }) {
			...
			try {
				...
				if (data.status === "sukses") {
					...
					if (myUser) {
						...
						setRoleProfil(
							myUser.role === "admin_dui" || myUser.role === "dui"
								? "DUI"
								: myUser.role,
						);
					}
	...
	const handleSearch = (e) => {
		if (e.key === "Enter" && searchQuery.trim() !== "") {
			...
			if (query.includes("monitoring") || query.includes("wilayah")) {
				...
				query.includes("kpi") ||
				query.includes("peringkat")
			) {
				navigate("/dui/leaderboard");
			} else if (
				query.includes("laporan") ||
				query.includes("grafik") ||
	...
	return (
		<div className="min-h-screen bg-[#F5EFE6] flex font-sans animate-fade-in">
			<aside
				className={`w-72 bg-[#0B4D1E] text-white flex flex-col fixed h-full z-40 transition-transform duration-500 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
			>
				<div className="p-8 flex items-center gap-3">
					...
					<div>
						...
						<p className="text-xs text-green-200 uppercase tracking-widest">
							DUI Dashboard
						</p>
					</div>
				</div>
					...
					{menuItems.map((item) => {
						...
						return (
							<Link
								key={item.name}
								to={item.path}
								className={`flex items-center gap-4 px-6 py-3.5 rounded-2xl transition-all duration-300 font-medium ${isActive ? "bg-[#F4A300] text-white shadow-lg" : "text-green-100 hover:bg-[#083a16] hover:text-white"}`}
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									className="h-5 w-5"
			...
			<div
				className={`flex-1 flex flex-col transition-all duration-500 ease-in-out ${isSidebarOpen ? "ml-72" : "ml-0"}`}
			>
					...
					<div className="flex items-center gap-6">
							...
							{isNotifOpen && (
								<div className="absolute top-full right-0 mt-4 w-80 bg-white rounded-3xl shadow-[0_15px_40px_rgba(0,0,0,0.12)] border border-gray-100 overflow-hidden z-50 animate-fade-in-up">
									<div className="p-5 border-b border-gray-100">
										<h3 className="font-extrabold text-[#0B4D1E] text-lg">
											Notifikasi
										</h3>
									...
									<div className="max-h-80 overflow-y-auto custom-scrollbar">
										{dataNotif.length > 0 ? (
											dataNotif.map((item) => (
												<div
													key={item.id}
													className="p-5 border-b border-gray-50 hover:bg-[#FDF6EA] cursor-pointer flex gap-4 transition-colors"
												>
													<div className="mt-1.5 flex-shrink-0">
														{!item.isRead ? (
															<div className="w-2.5 h-2.5 rounded-full bg-[#F4A300]"></div>
														) : (
															<div className="w-2.5 h-2.5"></div>
														)}
													</div>
													<div>
														<p className="text-sm font-bold text-[#0B4D1E]">
														...
														<p className="text-[10px] text-gray-400 mt-1.5 font-bold">
															{formatWaktuNotif(item.waktu || item.created_at)}
														</p>
													</div>
												</div>
											))
									...
									<div className="p-4 text-center bg-white border-t border-gray-100 hover:bg-gray-50 transition-colors">
										<button
											...
											className="text-[#0B4D1E] font-bold text-sm hover:text-[#F4A300] flex items-center justify-center gap-2 w-full transition-colors"
										>
						...
						>
							<div className="text-right hidden md:block">
								<p className="font-extrabold text-[#0B4D1E] text-sm">
									{namaProfil}
								</p>
								<p className="text-xs text-[#F4A300] font-bold bg-[#F4A300]/10 inline-block px-3 py-1 rounded-full mt-1">
									{roleProfil}
								</p>
							</div>
				...
				<footer className="py-8 text-center text-sm border-t border-gray-200/50 mt-10">
					<p className="font-semibold text-[#0B4D1E]/70 text-lg mb-1">
						SIM-TH © 2026
					</p>
					<p className="text-[#0B4D1E]/50 font-medium">
						Developed for Program Tabung Hijau IPB University
					</p>
				</footer>
			</div>
		</div>
