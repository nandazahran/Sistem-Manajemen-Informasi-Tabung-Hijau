	...
	return (
		<AdminLayout>
			...
			<div className="bg-gradient-to-r from-[#F5EFE6] via-[#F5EFE6] to-[#8FA57A]/30 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between shadow-sm relative overflow-hidden mt-2 mb-8 border border-white/60">
				<div className="z-10 max-w-xl mb-6 md:mb-0">
					<h2 className="text-4xl font-extrabold text-[#0B4D1E] mb-4">
						Selamat Datang, Admin SIM-TH{" "}
						<span className="text-green-600">🌱</span>
					</h2>
					<p className="text-gray-700 font-medium text-lg mb-8">
						Pantau dan kelola seluruh aktivitas Tabung Hijau IPB secara
						terpusat.
					</p>
					<button
			...
			<div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 mt-8">
				<h3 className="text-[#0B4D1E] font-extrabold text-xl mb-6">
					Quick Actions
				</h3>
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
					{[
						...
						},
						{
							t: "Tambah Kategori",
							s: "Kategori sampah",
							p: "/admin/kelola-kategori",
							i: "M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4",
						},
						{
							t: "Export Laporan",
							s: "Unduh data",
							p: "/admin/laporan",
							i: "M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1",
						},
						{
							t: "Leaderboard KPI",
							s: "Lihat ranking",
							p: "/admin/leaderboard",
							i: "M5 3v4M3 5h4M6 17v4",
						},
					].map((act, i) => (
						<button
							key={i}
						...
						>
							...
							<div className="text-left">
								...
								<p className="text-green-100/60 text-[10px] font-medium">
									{act.s}
								</p>
							</div>
						</button>
				...
				<div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
					...
					<div className="space-y-6">
						{aktivitasTerbaru.map((item, i) => (
							...
							>
								<div
									className={`${item.c} p-3 rounded-full transition-transform group-hover:scale-110`}
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										className="h-5 w-5"
						...
						{aktivitasTerbaru.length === 0 && (
							<p className="text-sm text-gray-500 py-4">Belum ada aktivitas.</p>
						)}
					</div>
				</div>
					...
					<div className="space-y-4">
						{riwayatTerbaru.map((rw, i) => {
							...
							return (
								<div
									key={i}
									className="flex justify-between items-center bg-[#F5EFE6] p-5 rounded-2xl hover:bg-white hover:shadow-lg transition-all border border-transparent hover:border-gray-100 cursor-pointer group"
								>
									<div>
										...
										<p className="text-[11px] text-gray-400 font-bold mt-1">
											{rw.berat / 1000} kg • {tglStr}
										</p>
									</div>
									<div className="font-extrabold text-[#0B4D1E] group-hover:text-green-600 transition-colors">
										{formatRp(rw.total_nilai)}
									</div>
								</div>
							);
						})}
