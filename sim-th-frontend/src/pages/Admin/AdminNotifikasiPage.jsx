			...
			<div className="bg-[#125B2A] rounded-[2rem] p-10 flex flex-col md:flex-row items-start md:items-center justify-between shadow-sm mt-2 mb-8 border border-white/60">
				<div className="flex items-center gap-5 text-white mb-6 md:mb-0">
					<div className="bg-[#F4A300] p-4 rounded-2xl relative shadow-md">
						...
						{unreadCount > 0 && (
							<span className="absolute -top-2 -right-2 bg-red-500 text-white text-[11px] font-bold w-6 h-6 rounded-full flex items-center justify-center border-2 border-[#125B2A] shadow-sm">
								{unreadCount}
							</span>
						)}
					</div>
			...
			<div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row gap-4">
				...
				<div className="relative w-48">
					<select
						...
						className="w-full bg-[#F5EFE6] text-[#0B4D1E] font-bold px-5 py-4 rounded-2xl outline-none cursor-pointer appearance-none transition-shadow focus:ring-2 focus:ring-[#0B4D1E]"
					>
						<option>Semua</option>
					...
					{isLoading ? (
						...
						filteredNotifications.map((notif) => (
							<div
								...
								className={`flex justify-between items-center p-6 rounded-3xl transition-all cursor-pointer ${notif.read ? "bg-white border border-gray-100 hover:shadow-sm" : "bg-[#FDF6EA] border border-[#F4A300]/30 shadow-sm"}`}
							>
								...
								{!notif.read && (
									<div className="w-2.5 h-2.5 bg-[#F4A300] rounded-full mr-2 shadow-sm"></div>
								)}
							</div>
						))
					) : (
