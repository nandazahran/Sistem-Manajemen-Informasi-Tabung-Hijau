	...
	return (
		<AdminLayout>
			...
			<div className="bg-[#0B4D1E] rounded-[2rem] p-10 flex items-center gap-5 text-white mt-2 mb-8 shadow-sm">
				...
				<div>
					...
					<p className="text-green-100/80 font-medium">
						Timeline lengkap aktivitas dan event sistem
					</p>
				</div>
			</div>
			...
			<div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 mb-8">
				<h3 className="font-extrabold text-xl text-[#0B4D1E] mb-6 border-b border-gray-100 pb-4">
					Seluruh Aktivitas
				</h3>
				{isLoading ? (
					...
					<div className="space-y-6">
						{aktivitasList
							.filter((a) => {
								...
								// 2. Date Filter
								if (filterWaktu === "Semua Waktu" || !a.tanggalAsli)
									return true;
							...
							.map((item, idx) => (
								<div key={item.id || idx} className="flex gap-4">
									<div
										className={`${item.warnaBg} p-3 rounded-full ${item.warnaTeks} h-12 w-12 flex items-center justify-center hover:scale-110 transition-transform`}
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											className="h-6 w-6"
