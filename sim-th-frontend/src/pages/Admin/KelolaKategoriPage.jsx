  ...
  return (
    <AdminLayout>
              ...
              {filteredCategories.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-8 py-5 flex justify-center gap-3">
                    <button onClick={() => { setSelectedCat(c); setIsEditOpen(true); }} className="p-2 text-gray-400 hover:text-[#F4A300] hover:bg-[#FDF6EA] rounded-lg transition-all" title="Edit">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg>
                    </button>
                    <button onClick={() => { setSelectedCat(c); setIsDeleteOpen(true); }} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Hapus">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">Total Kategori</p><h3 className="text-3xl font-extrabold text-[#0B4D1E]">{categories.length}</h3></div>
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">Rata-rata Harga</p><h3 className="text-3xl font-extrabold text-green-600">Rp 3.750</h3></div>
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">Total Setoran</p><h3 className="text-3xl font-extrabold text-[#0B4D1E]">2200 kg</h3></div>
      </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl relative border border-gray-100">
            ...
            <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); setIsAddOpen(false); setIsEditOpen(false); }}>
              <div>
                ...
                <input type="text" placeholder="Contoh: Botol Plastik" className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" defaultValue={isEditOpen ? selectedCat?.name : ''} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" placeholder="Contoh: 4500" className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" defaultValue={isEditOpen ? selectedCat?.harga.replace('.','') : ''} />
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl text-center">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={confirmDelete} className="flex-1 bg-red-500 text-white py-3.5 rounded-xl font-bold hover:bg-red-600 shadow-md transition-all">Ya, Hapus</button>
            </div>
          </div>
        </div>
      )}
