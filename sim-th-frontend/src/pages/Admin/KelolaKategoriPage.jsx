...
function KelolaKategoriPage() {
	...
	const avgHarga =
		categories.length > 0
			? categories.reduce((acc, c) => acc + c.harga_raw, 0) / categories.length
			: 0;
      ...
      `}</style>
			...
			<div className="bg-[#0B4D1E] rounded-[2rem] p-10 flex flex-col md:flex-row items-start md:items-center justify-between shadow-sm mt-2 mb-8">
				<div className="flex items-center gap-5 text-white mb-6 md:mb-0">
					...
					<div>
						...
						<p className="text-green-100/80 font-medium">
							Manajemen kategori dan harga sampah
						</p>
					</div>
				</div>
				<div className="flex gap-4">
					...
					<button
						...
						className="bg-[#F4A300] text-white px-6 py-3.5 rounded-2xl font-bold flex items-center gap-2 hover:bg-[#d68e00] transition-all shadow-md"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							className="h-5 w-5"
				...
				<div className="relative w-full md:w-64">
					...
					{isJenisOpen && (
						<div className="absolute top-full mt-2 w-full bg-white border border-gray-100 shadow-xl rounded-xl z-50 overflow-hidden py-1">
							{["Semua Jenis", "Daur Ulang", "Organik", "Non-organik"].map(
								(jenis) => (
									<div
										key={jenis}
										onClick={() => {
								...
								filteredCategories.map((c) => (
									<tr key={c.id} className="hover:bg-gray-50 transition-colors">
										<td className="px-8 py-5 flex items-center gap-3">
											...
											<span className="font-extrabold text-[#0B4D1E]">
												{c.name}
											</span>
										</td>
										<td className="px-8 py-5">
											<span
												className={`text-[10px] px-3 py-1 rounded-full font-bold ${c.jenis === "Organik" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}
											>
												{c.jenis}
											</span>
										</td>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
				...
				<div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
					...
					<h3 className="text-3xl font-extrabold text-[#0B4D1E]">-</h3>
				</div>
			</div>
				...
				<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
					<div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl relative border border-gray-100">
						...
						<form
							...
							onSubmit={(e) => {
								e.preventDefault();
								confirmAction(isEditOpen ? "edit" : "add");
							}}
						>
							<div>
							...
							<div>
								...
								<div className="relative">
									<select
										...
										defaultValue={
											isEditOpen ? selectedCat?.jenis : "Daur Ulang"
										}
									>
							...
							<div className="flex gap-4 mt-8 pt-4">
								...
								<button
									type="submit"
									className="flex-1 bg-[#125B2A] text-white py-4 rounded-2xl font-bold hover:bg-[#0B4D1E] shadow-md transition-all flex items-center justify-center gap-2"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										className="h-5 w-5"
			...
			{isDeleteOpen && selectedCat && (
				<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
					<div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl text-center">
						...
						<h3 className="text-2xl font-extrabold text-[#0B4D1E] mb-2">
							Hapus Kategori?
						</h3>
						<p className="text-gray-500 font-medium mb-8 text-sm">
							Kategori <b>{selectedCat.name}</b> akan dihapus secara permanen.
						</p>
						<div className="flex gap-4">
							<button
								type="button"
			...
			{toastMessage && (
				<div
					...
					className="bg-[#E8F5E9] text-[#2E7D32] border border-[#A5D6A7] px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3"
				>
