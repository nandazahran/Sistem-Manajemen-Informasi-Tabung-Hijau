    ...
    <AdminLayout>
      ...
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 mb-8">
        <div className="relative mb-6">
          ...
          <input type="text" placeholder="Cari kategori sampah..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#F5EFE6] px-14 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#F5EFE6] text-[#0B4D1E]">
              <tr><th className="px-6 py-5 font-bold rounded-l-xl">Kategori</th><th className="px-6 py-5 font-bold">Jenis</th><th className="px-6 py-5 font-bold">Harga / kg</th><th className="px-6 py-5 font-bold">Total Setoran</th><th className="px-6 py-5 font-bold">Update Terakhir</th><th className="px-6 py-5 font-bold text-center rounded-r-xl">Aksi</th></tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredCategories.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-5 flex items-center gap-3"><div className="bg-[#EAE5DA] p-2 rounded-lg text-[#0B4D1E]"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg></div><span className="font-extrabold text-[#0B4D1E]">{c.name}</span></td>
                  <td className="px-6 py-5"><span className="bg-green-100 text-green-700 text-[10px] px-3 py-1 rounded-full font-bold">{c.jenis}</span></td><td className="px-6 py-5 font-extrabold text-green-600">Rp {c.harga}</td><td className="px-6 py-5 font-bold text-[#0B4D1E]">{c.setoran}</td><td className="px-6 py-5 text-gray-400 text-sm font-medium">{c.update}</td>
