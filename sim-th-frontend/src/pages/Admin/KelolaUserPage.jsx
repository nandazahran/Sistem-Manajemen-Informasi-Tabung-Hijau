...
function KelolaUserPage() {
  ...
  const [formData, setFormData] = useState({ 
    username: '', nama: '', email: '', roleType: 'bem_wilayah', wilayahNama: '', password: '' 
  });
  ...
  const filteredUsers = users.filter(u => {
    ...
    const matchRole = filterRole === 'Semua Role' || getRoleLabel(u.role) === filterRole;
  ...
  const totalAdmin = users.filter(u => u.role === 'admin' || u.role === 'bem_km').length;
  const totalBEM = users.filter(u => u.role && u.role.startsWith('bem_') && u.role !== 'bem_km').length;
  ...
  const handleSubmitAdd = async () => {
    if (!formData.username || !formData.nama || !formData.email || !formData.password) {
  ...
  const handleGenerateSeed = async () => {
    if (!window.confirm("Apakah kamu yakin ingin men-generate data dummy selama 4 bulan terakhir? (Perlu beberapa saat)")) return;
    setSubmitting(true);
  ...
  const detectRoleType = (roleCode) => {
    if (['admin', 'dui', 'bem_km', 'ormawa_ppku'].includes(roleCode)) return roleCode;
  ...
  const closeModals = () => {
    ...
    setIsDeleteConfirmOpen(false);
    setSelectedUser(null);
    setFormData({ username: '', nama: '', email: '', roleType: 'bem_wilayah', wilayahNama: '', password: '' });
  };
    ...
    <AdminLayout>
      ...
      <div className="bg-[#0B4D1E] rounded-[2rem] p-10 flex flex-col md:flex-row items-start md:items-center justify-between shadow-sm mt-2 mb-8">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          {isSuperAdmin && (
            <button onClick={handleGenerateSeed} disabled={submitting} className="bg-white text-[#0B4D1E] px-8 py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-gray-100 transition-all shadow-md disabled:opacity-50">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
              {submitting ? 'Proses...' : '🛠️ Data Dummy'}
            </button>
          )}
          <button onClick={() => setIsAddOpen(true)} className="bg-[#F4A300] text-white px-8 py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-[#d68e00] transition-all shadow-md">
      ...
      <div className="bg-white p-4 rounded-[2rem] shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="relative flex-1 w-full">
          ...
          <input type="text" placeholder="Cari nama, email, atau username..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#F5EFE6] px-14 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] transition-shadow" />
        </div>
          ...
          <div className="relative w-full md:w-48">
            ...
            {isRoleDropdownOpen && (
              <div className="absolute top-full mt-2 w-full bg-white border border-gray-100 shadow-xl rounded-xl z-50 overflow-hidden py-1">
                {['Semua Role', 'Admin', 'BEM Wilayah', 'DUI', 'BEM KM', 'Ormawa'].map(r => (
                  <div 
                    key={r} 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan="4" className="text-center py-10 text-gray-400 font-medium">
                  <div className="flex items-center justify-center gap-3">
                    <svg className="animate-spin h-5 w-5 text-[#0B4D1E]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                    Memuat data user...
                  </div>
                </td></tr>
              ) : filteredUsers.length === 0 ? (
                <tr><td colSpan="4" className="text-center py-10 text-gray-400 font-medium">Tidak ada user yang ditemukan.</td></tr>
              ) : (
                filteredUsers.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-8 py-5">
                        <p className="font-extrabold text-[#0B4D1E]">{u.nama}</p>
                        <p className="text-gray-400 text-xs font-medium">{u.email}</p>
                        <p className="text-gray-300 text-[10px] font-mono mt-0.5">@{u.username}</p>
                      </td>
                      <td className="px-8 py-5">
                        <span className={`${badge.bg} ${badge.text} text-[10px] px-3 py-1 rounded-full font-bold`}>{getRoleLabel(u.role)}</span>
                        <p className="text-gray-400 text-xs font-medium mt-1">{u.nama_wilayah || '-'}</p>
                      </td>
                      <td className="px-8 py-5">
                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold ${u.status === 'Aktif' ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-red-50 text-red-600'}`}>
                          {u.status}
                        </span>
                      </td>
                      <td className="px-8 py-5 flex items-center justify-center gap-3">
                        ...
                        {/* BUTTON HAPUS */}
                        <button onClick={() => openDeleteConfirm(u)} title="Hapus User" className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">Total User</p><h3 className="text-3xl font-extrabold text-[#0B4D1E]">{totalUser}</h3></div>
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">Admin</p><h3 className="text-3xl font-extrabold text-[#F4A300]">{totalAdmin}</h3></div>
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">BEM Wilayah</p><h3 className="text-3xl font-extrabold text-[#0B4D1E]">{totalBEM}</h3></div>
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100"><p className="text-gray-400 text-sm font-medium mb-1">User Aktif</p><h3 className="text-3xl font-extrabold text-[#2E7D32]">{totalAktif}</h3></div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative animate-fade-in-up border border-gray-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Username</label>
                <input type="text" placeholder="Contoh: ahmad_fateta" value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] transition-shadow placeholder-gray-400" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Nama Lengkap</label>
                <input type="text" placeholder="Masukkan nama lengkap" value={formData.nama} onChange={(e) => setFormData({...formData, nama: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] transition-shadow placeholder-gray-400" />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={formData.roleType} onChange={(e) => setFormData({...formData, roleType: e.target.value, wilayahNama: ''})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] appearance-none cursor-pointer">
              ...
              {needsWilayah && (
                <div>
                  ...
                  <div className="relative">
                    <select value={formData.wilayahNama} onChange={(e) => setFormData({...formData, wilayahNama: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] appearance-none cursor-pointer">
            ...
            <div className="flex gap-4 mt-8">
              ...
              <button onClick={handleSubmitAdd} disabled={submitting} className="flex-1 bg-[#125B2A] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-[#0B4D1E] shadow-md transition-all disabled:opacity-50">
                {submitting ? (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative animate-fade-in-up border border-gray-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Nama Lengkap</label>
                <input type="text" value={formData.nama} onChange={(e) => setFormData({...formData, nama: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] transition-shadow" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Email</label>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={formData.status || 'Aktif'} onChange={(e) => setFormData({...formData, status: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] appearance-none cursor-pointer">
              ...
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Telepon</label>
                <input type="text" placeholder="08xxxxxxxxxx" value={formData.telepon || ''} onChange={(e) => setFormData({...formData, telepon: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E] transition-shadow placeholder-gray-400" />
              </div>
            </div>

            <div className="flex gap-4 mt-8">
              <button onClick={closeModals} className="flex-1 bg-[#F5EFE6] text-[#0B4D1E] py-4 rounded-2xl font-bold hover:bg-[#EAE5DA] transition-all">Batal</button>
              <button onClick={handleSubmitEdit} disabled={submitting} className="flex-1 bg-[#125B2A] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-[#0B4D1E] shadow-md transition-all disabled:opacity-50">
                {submitting ? (
                  <svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
          ...
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl relative animate-fade-in-up border border-gray-100">
            <div className="flex flex-col items-center text-center mb-6">
              <div className="bg-red-50 p-4 rounded-full text-red-500 mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
              </div>
              <h3 className="text-xl font-extrabold text-[#0B4D1E] mb-2">Hapus User?</h3>
              <p className="text-gray-500 text-sm">Apakah kamu yakin ingin menghapus akun <span className="font-bold text-[#0B4D1E]">{selectedUser.nama}</span>? Tindakan ini tidak bisa dibatalkan.</p>
            </div>
            <div className="flex gap-4">
              <button onClick={closeModals} className="flex-1 bg-[#F5EFE6] text-[#0B4D1E] py-4 rounded-2xl font-bold hover:bg-[#EAE5DA] transition-all">Batal</button>
              <button onClick={handleDeleteUser} disabled={submitting} className="flex-1 bg-red-500 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-red-600 shadow-md transition-all disabled:opacity-50">
                {submitting ? (
                  <svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
      ...
      {/* TOAST NOTIFICATION */}
      {toastMessage && (
        <div style={{ animation: 'fadeInDown 0.3s ease-out' }} className={`fixed top-10 right-10 z-[99999] ${toastType === 'error' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-[#E8F5E9] text-[#2E7D32] border-[#A5D6A7]'} border px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3`}>
          <div className={`${toastType === 'error' ? 'bg-red-500' : 'bg-[#2E7D32]'} text-white rounded-full p-1`}>
            {toastType === 'error' ? (
