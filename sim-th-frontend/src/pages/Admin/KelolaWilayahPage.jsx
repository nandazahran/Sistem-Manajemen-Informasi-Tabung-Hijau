...
function KelolaWilayahPage() {
	...
	const fetchData = async () => {
		try {
			...
			const lbRes = await fetch(`${baseUrl}/dashboard/leaderboard`, {
				headers,
			});
	...
	const filteredWilayah = wilayahData.filter((w) => {
		...
		const matchStatus =
			filterStatus === "Semua Status" || w.status === filterStatus;
		...
		<AdminLayout>
				...
				<div className="flex items-center gap-5 text-white mb-6 md:mb-0">
					...
					<div>
						...
						<p className="text-green-100/80 font-medium">
							Monitoring dan manajemen wilayah BEM Fakultas
						</p>
					</div>
				</div>
				<button
			...
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
				{filteredWilayah.map((w) => (
					<div
						key={w.id}
						className="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 flex flex-col hover:shadow-xl transition-all duration-300"
					>
						{/* Header Card */}
						<div className="flex justify-between items-start mb-6">
							<div className="flex items-center gap-4">
								...
								<div>
									<h3 className="font-extrabold text-lg text-[#0B4D1E]">
										{w.name}
									</h3>
							...
							<span
								className={`px-3 py-1 rounded-full text-[10px] font-bold ${w.status === "Aktif" ? "bg-[#E8F5E9] text-[#2E7D32]" : "bg-[#FFEBEE] text-[#C62828]"}`}
							>
								{w.status}
							</span>
						</div>
						...
						<div className="grid grid-cols-2 gap-3 mb-6 flex-1">
							<div className="bg-[#F5EFE6] p-4 rounded-2xl">
								...
								<p className="font-extrabold text-[#0B4D1E] text-lg">
									{w.kontribusi} kg
								</p>
							</div>
							<div className="bg-[#F5EFE6] p-4 rounded-2xl">
								...
								<p className="font-extrabold text-[#F4A300] text-lg">{w.kpi}</p>
							</div>
							<div className="bg-[#F5EFE6] p-4 rounded-2xl">
								<p className="text-[10px] text-gray-500 font-medium mb-1">
									Total Transaksi
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
				...
				<div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
					...
					<h3 className="text-3xl font-extrabold text-[#F4A300]">{avgKpi}</h3>
				</div>
			</div>
					...
					<div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative animate-fade-in-up border border-gray-100">
						...
						<div className="space-y-4">
							...
							<div>
								...
								<input
									type="email"
									placeholder="bem.fateta@ipb.ac.id"
					...
					<div className="bg-white w-full max-w-md rounded-[2rem] p-8 shadow-2xl relative animate-fade-in-up border border-gray-100">
						...
						<div className="flex flex-col">
							...
							<div className="py-4 border-b border-gray-100">
								...
								<span className="font-medium text-[#125B2A] text-sm leading-relaxed">
									{selectedWilayah.desc}
								</span>
							</div>
							<div className="flex justify-between items-center py-4 border-b border-gray-100">
								<span className="text-gray-400 font-medium text-sm">
								...
								<span className="font-extrabold text-[#F4A300] text-sm">
									{selectedWilayah.ranking}
								</span>
							</div>
							<div className="flex justify-between items-center py-4 border-b border-gray-100">
								<span className="text-gray-400 font-medium text-sm">
							...
							<div className="flex justify-between items-center py-4 border-b border-gray-100">
								...
								<span
									className={`px-3 py-1 rounded-full text-xs font-bold ${selectedWilayah.status === "Aktif" ? "bg-[#E8F5E9] text-green-700" : "bg-yellow-100 text-yellow-700"}`}
								>
				...
				<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
					<div className="bg-white p-8 rounded-[2rem] shadow-2xl max-w-sm w-full text-center animate-fade-in-up border border-gray-100">
						...
						<p className="text-sm text-gray-500 font-medium mb-8">
							Apakah Anda yakin ingin menghapus <b>{selectedWilayah.name}</b>?
			...
			{toastMessage && (
				<div
					...
					className="fixed top-10 right-10 z-[99999] bg-[#E8F5E9] text-[#2E7D32] border border-[#A5D6A7] px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3"
				>
