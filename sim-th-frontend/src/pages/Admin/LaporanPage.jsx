...
function LaporanPage() {
	...
	const filteredData = dataTrx.filter((t) => {
		const matchBulan =
			filterBulanUtama === "Semua Periode" ||
			getBulanPeriode(t.tanggal) === filterBulanUtama;
	...
	const summaryCards = [
		{
			t: "Total Transaksi",
			v: totalTransaksi.toLocaleString("id-ID"),
		...
		{
			t: "Wilayah Aktif",
			v: activeWilayahCount.toString(),
			b: `dari ${wilAktifCount} total wilayah`,
		},
	];
	...
	const filteredRekap = Object.keys(wilayahMap)
		.map((w, idx) => {
	...
	const pieData = Object.keys(pieMap).map((k) => ({
		name: k,
		value: pieMap[k],
	}));
	...
	const generateLineChart = () => {
		...
		if (filterBulanUtama !== "Semua Periode") {
			if (filterBulanUtama.startsWith("Jan")) {
				startMonth = 0;
				endMonth = 1;
			} else if (filterBulanUtama.startsWith("Mar")) {
				startMonth = 2;
				endMonth = 3;
			} else if (filterBulanUtama.startsWith("Mei")) {
				startMonth = 4;
				endMonth = 5;
			} else if (filterBulanUtama.startsWith("Jul")) {
				startMonth = 6;
				endMonth = 7;
			} else if (filterBulanUtama.startsWith("Sep")) {
				startMonth = 8;
				endMonth = 9;
			} else if (filterBulanUtama.startsWith("Nov")) {
				startMonth = 10;
				endMonth = 11;
			}
		} else {
	...
	const handleExport = async (e) => {
		...
		try {
			...
			XLSX.writeFile(
				workbook,
				`Laporan_Admin_SIMTH_${exportPeriode.replace(/ /g, "")}_${exportYear}.xlsx`,
			);
		} catch (error) {
	...
	return (
		<AdminLayout>
			...
			<div className="bg-white p-4 rounded-[2rem] shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row gap-4 items-center">
					...
					{isWilayahDropdownOpen && (
						<div className="absolute top-full mt-2 w-full bg-white border border-gray-100 shadow-xl rounded-xl z-50 overflow-hidden py-1 max-h-60 overflow-y-auto">
							<div
								...
								className={`px-5 py-2.5 cursor-pointer text-sm font-medium transition-colors ${selectedWilayahId === "Semua Wilayah" ? "bg-[#0B4D1E] text-white" : "text-[#0B4D1E] hover:bg-gray-100"}`}
							>
								Semua Wilayah
							</div>
							{listWilayah.map((wil) => (
								<div
									...
									className={`px-5 py-2.5 cursor-pointer text-sm font-medium transition-colors ${selectedWilayahId === wil.id.toString() ? "bg-[#0B4D1E] text-white" : "text-[#0B4D1E] hover:bg-gray-100"}`}
								>
									{wil.nama}
								</div>
							))}
				...
				<div className="relative w-full md:w-64">
					<div
						onClick={() => setIsBulanUtamaOpen(!isBulanUtamaOpen)}
						className="bg-[#F5EFE6] text-[#0B4D1E] font-bold px-5 py-4 rounded-2xl cursor-pointer flex justify-between items-center hover:bg-[#EAE5DA] transition-colors h-[56px]"
					>
						...
						<div className="absolute top-full mt-2 w-full bg-white border border-gray-100 shadow-xl rounded-xl z-50 overflow-hidden py-1">
							{periodeOptions.map((opt) => (
								<div
									key={opt}
									onClick={() => {
										setFilterBulanUtama(opt);
										setIsBulanUtamaOpen(false);
									}}
									className={`px-5 py-2.5 cursor-pointer text-sm font-medium transition-colors ${filterBulanUtama === opt ? "bg-[#0B4D1E] text-white" : "text-[#0B4D1E] hover:bg-gray-100"}`}
								>
									{opt}
								</div>
							))}
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
				{summaryCards.map((c, i) => (
					<div
						key={i}
						className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 hover:-translate-y-1 transition-transform"
					>
						...
						<table className="w-full text-left border-collapse">
							<thead className="bg-[#F5EFE6] text-[#0B4D1E]">
								<tr>
									...
									<th className="px-8 py-5 font-bold rounded-r-xl">KPI</th>
								</tr>
							</thead>
					...
					<div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl relative border border-gray-100">
						...
						<form onSubmit={handleExport} className="space-y-6">
							...
							<div className="flex gap-4 mt-8 pt-2">
								...
								<button
									type="submit"
									className="flex-1 bg-[#125B2A] text-white py-4 rounded-2xl font-bold hover:bg-[#0B4D1E] shadow-md flex items-center justify-center gap-2 transition-all"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										className="h-5 w-5"
