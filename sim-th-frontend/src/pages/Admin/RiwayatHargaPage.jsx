		...
		const fetchRiwayat = async () => {
			...
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/riwayat-harga`,
					{
			...
			<div className="bg-[#0B4D1E] rounded-[2rem] p-10 flex items-center gap-5 text-white mt-2 mb-8 shadow-sm">
				...
				<div>
					...
					<p className="text-green-100/80 font-medium">
						Semua perubahan harga kategori sampah
					</p>
				</div>
			</div>
				...
				<table className="w-full text-left border-collapse">
					<thead className="bg-[#F5EFE6] text-[#0B4D1E]">
						<tr>
							...
							<th className="px-8 py-5 font-bold">Harga Lama</th>
							<th className="px-8 py-5 font-bold">Harga Baru</th>
							<th className="px-8 py-5 font-bold">Perubahan</th>
							<th className="px-8 py-5 font-bold">Admin</th>
						</tr>
					</thead>
					<tbody className="divide-y divide-gray-100">
						{isLoading ? (
							<tr>
								<td
									colSpan="6"
									className="text-center py-10 text-gray-500 font-medium animate-pulse"
								>
							...
							riwayatData.map((r, i) => (
								<tr key={r.id || i} className="hover:bg-gray-50">
									...
									<td
										className={`px-8 py-5 font-bold ${r.drop ? "text-red-500" : "text-green-500"}`}
									>
										{r.change}
									</td>
