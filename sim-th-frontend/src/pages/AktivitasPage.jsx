    ...
    <DashboardLayout>
      ...
      <div className="flex gap-4 mb-8 h-[60px]">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Cari aktivitas..." className="w-full h-full bg-white px-14 rounded-xl border border-gray-100 focus:outline-none focus:ring-2 focus:ring-[#0B4D1E] font-medium" />
        </div>
        ...
        <div className="relative min-w-[160px]">
          <button 
            ...
            className="w-full h-full bg-white px-8 rounded-xl border border-gray-100 text-gray-600 font-bold hover:text-[#0B4D1E] transition-colors flex items-center justify-center gap-2"
          >
          ...
          {isFilterOpen && (
            <div className="absolute top-full right-0 mt-2 w-56 bg-white border border-gray-100 shadow-xl rounded-2xl overflow-hidden z-50">
              {filterOptions.map(opt => (
                <div 
                  key={opt} 
                  onClick={() => { setFilterWaktu(opt); setIsFilterOpen(false); }} 
                  className={`px-5 py-3.5 cursor-pointer text-sm font-bold transition-colors ${filterWaktu === opt ? 'bg-[#0B4D1E] text-white' : 'text-[#0B4D1E] hover:bg-[#F5EFE6]'}`}
                >
                  {opt}
                </div>
              ))}
