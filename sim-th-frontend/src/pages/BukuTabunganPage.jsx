	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				if (
					activeWilayahId &&
					dataWil.status === "sukses" &&
					Array.isArray(dataWil.data)
				) {
				...
				if (
					dataTrx.status === "sukses" &&
					Array.isArray(dataTrx.data) &&
					dataTrx.data.length > 0
				) {
					const allTrx = dataTrx.data;
					// Sortir transaksi dari yang terbaru ke terlama
					const sortedTrx = [...allTrx].sort(
					...
					sortedTrx.forEach((t) => {
						...
						if (
							trxDate.getMonth() === lastMonth &&
							trxDate.getFullYear() === lastMonthYear
						) {
					...
					setRataRataTrx(
						sortedTrx.length > 0 ? totalAll / sortedTrx.length : 0,
					);
				...
				if (
					dataTabungan.status === "sukses" &&
					Array.isArray(dataTabungan.data)
				) {
					// Cari dompet tabungan berdasarkan wilayah_id yang akurat
					const myDompet = dataTabungan.data.find(
						(t) => t.wilayah_id === activeWilayahId,
					);
					if (myDompet) {
						...
						try {
							const resHistori = await fetch(
								`${import.meta.env.VITE_API_URL}/tabungan/histori?wilayah_id=${myDompet.wilayah_id}`,
								{ headers },
							);
							const dataHistori = await resHistori.json();
							if (
								dataHistori.status === "sukses" &&
								Array.isArray(dataHistori.data) &&
								dataHistori.data.length > 0
							) {
		...
		<DashboardLayout>
				...
				<div className="flex items-center gap-5">
					...
					<div>
						...
						<p className="text-green-100/80 font-medium">
							Rekap pendapatan dari sampah wilayah
						</p>
					</div>
				</div>
			</div>
			...
			<div className="bg-gradient-to-r from-[#0B4D1E] to-[#146b2d] p-10 rounded-[2rem] text-white shadow-sm mb-6 flex justify-between items-center relative overflow-hidden">
				...
				<div className="z-10">
					...
					<p className="text-sm text-green-200">
						Wilayah: {namaWilayah} • Otomatis sinkron
					</p>
				</div>
				<div
					className={`bg-white/20 px-5 py-2.5 rounded-full font-bold backdrop-blur-sm border border-white/30 flex items-center gap-2 z-10 ${trenPertumbuhan < 0 ? "text-red-200" : "text-white"}`}
				>
					{trenPertumbuhan >= 0 ? (
						<svg
							xmlns="http://www.w3.org/2000/svg"
					...
					{trenPertumbuhan > 0 ? "+" : ""}
					{trenPertumbuhan.toFixed(1)}% bulan ini
				</div>
			</div>
					...
					{grafikSaldo && grafikSaldo.length > 0 ? (
						...
						<div className="w-full h-full flex items-center justify-center">
							<p className="text-gray-400 font-medium">
								Belum ada data transaksi yang cukup untuk membuat grafik
								wilayah.
							</p>
						</div>
					)}
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
				...
				<div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 hover:-translate-y-1 transition-all duration-300">
					...
					<h3
						className={`text-3xl font-extrabold ${trenPertumbuhan >= 0 ? "text-[#0B4D1E]" : "text-red-500"}`}
					>
						{trenPertumbuhan > 0 ? "+" : ""}
						{trenPertumbuhan.toFixed(1)}%
					</h3>
				</div>
			</div>
				...
				<div className="space-y-4">
					{riwayatPemasukan.map((item, idx) => (
						<div
							...
							className="flex justify-between items-center bg-[#F5EFE6] p-4 rounded-2xl hover:bg-white hover:shadow-md transition-all cursor-pointer border border-transparent hover:border-gray-200 group"
						>
							<div className="flex items-center gap-4">
								...
								<div>
									<p className="font-bold text-[#0B4D1E] group-hover:text-[#F4A300] transition-colors">
										Setoran {item.nama_kategori}
									</p>
							...
							<div className="font-extrabold text-green-600">
								+ {formatRp(item.total_nilai)}
							</div>
						</div>
					))}
					{riwayatPemasukan.length === 0 && (
						<p className="text-center text-sm font-bold text-gray-400 py-4">
							Belum ada pemasukan tabungan.
						</p>
					)}
				</div>
			</div>
						...
						<div className="space-y-5">
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								...
								<p className="font-extrabold text-[#0B4D1E]">
									{formatTanggal(selectedPemasukan.tanggal)}
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								<p className="text-gray-500 font-medium text-sm">
								...
								<span className="bg-[#EAE5DA] text-[#0B4D1E] px-4 py-1.5 rounded-full font-bold text-xs">
									{selectedPemasukan.nama_kategori}
								</span>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								<p className="text-gray-500 font-medium text-sm">
								...
								<p className="font-extrabold text-[#0B4D1E] text-base">
									{(selectedPemasukan.berat ||
										selectedPemasukan.berat_gram ||
										0) / 1000}{" "}
									kg
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								...
								<p className="font-extrabold text-green-600 text-lg">
									+ {formatRp(selectedPemasukan.total_nilai)}
								</p>
							</div>
							...
							<div className="pt-2 pb-2">
								...
								<div className="bg-[#F5EFE6] p-5 rounded-2xl">
									<p className="font-bold text-[#0B4D1E] text-sm leading-relaxed">
										{selectedPemasukan.catatan ||
											"Tidak ada catatan untuk setoran ini."}
									</p>
								</div>
							</div>
						</div>
