		...
		<AuthLayout title="Masuk" subtitle="Masuk untuk mengakses dashboard">
			<form onSubmit={handleLogin} className="flex flex-col gap-5">
				...
				<div className="flex justify-between items-center mt-1">
					<label className="flex items-center gap-2 cursor-pointer">
						<input
							...
							className="w-4 h-4 text-[#0A391D] rounded border-gray-300 focus:ring-[#0A391D]"
						/>
					...
					<Link
						to="/reset-password"
						className="text-sm font-bold text-[#F7941D] hover:underline"
					>
						Lupa password?
					</Link>
				</div>
				...
				<button
					type="submit"
					disabled={isLoading}
					className={`w-full text-white py-3.5 rounded-xl font-bold transition-all shadow-md mt-2 ${isLoading ? "bg-gray-400 cursor-not-allowed" : "bg-[#0A391D] hover:bg-[#072a15]"}`}
				>
			...
			<p className="mt-8 text-center text-sm text-gray-600">
				...
				<Link
					to="/register"
					className="text-[#0A391D] font-extrabold hover:underline"
				>
					Daftar di sini
				</Link>
			</p>
