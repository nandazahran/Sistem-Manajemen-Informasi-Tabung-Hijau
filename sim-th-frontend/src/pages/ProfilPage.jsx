...
function ProfilPage() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      alert('Terjadi kesalahan koneksi ke server, profil diperbarui secara lokal.');
      setFormData(editData);
      setIsEditModalOpen(false);
    } finally {
      setIsSavingProfile(false);
  ...
  const handleSavePassword = async () => {
    ...
    try {
      ...
      alert('Gagal menyambung ke server, simulasi pengubahan password berhasil.');
      setIsPasswordModalOpen(false);
      setPassData({ current: '', new: '', confirm: '' });
    } finally {
      setIsSavingPassword(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-[#F5EFE6] p-4 rounded-2xl flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="font-extrabold text-[#0B4D1E]">{formData.organisasi}</p>
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative border border-gray-100">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={editData.nama} onChange={(e) => setEditData({...editData, nama: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Email</label>
                <input type="email" value={editData.email} onChange={(e) => setEditData({...editData, email: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Nomor Telepon</label>
                <input type="text" value={editData.telepon} onChange={(e) => setEditData({...editData, telepon: e.target.value})} className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" />
              </div>
            </div>

            <div className="flex gap-4 mt-8">
              <button onClick={() => setIsEditModalOpen(false)} className="flex-1 bg-[#F5EFE6] text-[#0B4D1E] py-4 rounded-2xl font-bold">Batal</button>
              <button onClick={handleSaveProfile} disabled={isSavingProfile} className="flex-1 bg-[#125B2A] text-white py-4 rounded-2xl font-bold shadow-md flex items-center justify-center gap-2">
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl relative border border-gray-100">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type={showCurrent ? "text" : "password"} value={passData.current} onChange={(e) => setPassData({...passData, current: e.target.value})} placeholder="Masukkan password" className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Password Baru</label>
                <input type={showNew ? "text" : "password"} value={passData.new} onChange={(e) => setPassData({...passData, new: e.target.value})} placeholder="Minimal 8 karakter" className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0B4D1E] mb-2">Konfirmasi Password</label>
                <input type={showConfirm ? "text" : "password"} value={passData.confirm} onChange={(e) => setPassData({...passData, confirm: e.target.value})} placeholder="Ulangi password" className="w-full bg-[#F5EFE6] px-5 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none" />
              </div>
            </div>

            <div className="flex gap-4 mt-8">
              <button onClick={() => setIsPasswordModalOpen(false)} className="flex-1 bg-[#F5EFE6] text-[#0B4D1E] py-4 rounded-2xl font-bold">Batal</button>
              <button onClick={handleSavePassword} disabled={isSavingPassword} className="flex-1 bg-[#0B4D1E] text-white py-4 rounded-2xl font-bold shadow-md">
