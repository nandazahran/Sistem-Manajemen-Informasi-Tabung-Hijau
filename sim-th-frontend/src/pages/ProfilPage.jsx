...
function ProfilPage() {
  ...
  // Fungsi Logout
  const handleLogout = () => {
    // Di sini lu bisa tambahin fungsi hapus token (localStorage.removeItem('token'))
    navigate('/login');
  };
    ...
    <DashboardLayout>
      <div className="bg-[#0B4D1E] rounded-[2.5rem] p-12 flex items-center gap-8 text-white shadow-sm mt-2 relative overflow-hidden">
        ...
        <div className="relative group cursor-pointer z-10">
          <div className="w-28 h-28 bg-[#F4A300] rounded-3xl flex items-center justify-center text-4xl font-extrabold shadow-lg">BF</div>
      ...
      <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-100 mt-8 hover:-translate-y-1 transition-transform duration-300">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => setIsEditing(!isEditing)} className="bg-[#F5EFE6] text-[#0B4D1E] px-6 py-2.5 rounded-full font-bold hover:bg-[#EAE5DA] transition-colors flex items-center gap-2">
        ...
        <div className="space-y-6">
          {Object.keys(formData).map((key) => (
            <div key={key}>
              ...
              <input type="text" value={formData[key]} onChange={(e) => setFormData({...formData, [key]: e.target.value})} readOnly={!isEditing} className={`w-full px-5 py-4 rounded-2xl font-bold transition-all outline-none ${isEditing ? 'bg-white border-2 border-[#F4A300] text-[#0B4D1E]' : 'bg-[#F5EFE6] border-2 border-transparent text-[#0B4D1E]'}`} />
            </div>
          ))}
        </div>
      </div>
