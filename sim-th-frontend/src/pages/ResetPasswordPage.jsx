...
function ResetPasswordPage() {
	...
	const handleLanjutKePassword = (e) => {
		...
		if (kodeOtpLengkap.length < 6) {
			setPesan({
				tipe: "error",
				teks: "Masukkan 6 digit kode OTP dengan lengkap!",
			});
			return;
		}
	...
	const handleSubmitFinal = async (e) => {
		...
		try {
			...
			if (response.status === 200) {
				...
				setPesan({
					tipe: "error",
					teks: data.pesan || "Gagal meriset password",
				});
				setStep(1);
			}
		} catch (error) {
	...
	return (
		<AuthLayout
			...
			subtitle={
				step === 1
					? `Masukkan kode OTP yang dikirim ke ${emailTerdeteksi}`
		...
		>
			...
			{step === 1 && (
				<form onSubmit={handleLanjutKePassword} className="flex flex-col gap-6">
					...
					<button
						type="submit"
						className="w-full bg-[#0B4D1E] text-white py-3.5 rounded-xl font-bold hover:bg-[#0B4D1E] transition-all shadow-md"
					>
						Lanjut
					</button>
				</form>
