...
function RiwayatTransaksiPage() {
	...
	const filteredData = riwayatData.filter((item) => {
		...
		const matchKategori =
			...
			item.nama_kategori === filterKategori;
	...
	const totalBerat =
		filteredData.reduce(
			(sum, item) => sum + (item.berat || item.berat_gram || 0),
			0,
		) / 1000;
	const totalNilai = filteredData.reduce(
		(sum, item) => sum + (item.total_nilai || 0),
		0,
	);
			...
			<div className="bg-[#0B4D1E] rounded-[2rem] p-10 flex items-center gap-5 text-white shadow-sm mt-2 mb-8">
				...
				<div>
					...
					<p className="text-green-100/80 font-medium">
						Semua transaksi sampah wilayah Anda
					</p>
				</div>
			</div>
							...
							{isMonthPickerOpen && (
								<div className="absolute top-full mt-2 left-0 w-full bg-white p-4 rounded-3xl shadow-2xl border border-gray-100 z-30 grid grid-cols-2 gap-2">
									<button
										...
										className="col-span-2 py-3 rounded-xl text-xs font-bold bg-[#0B4D1E] text-white hover:bg-[#083a16] transition-all mb-1"
									>
										Reset Bulan
									</button>
							...
							filteredData.map((item) => (
								...
								>
									<td className="px-8 py-5 text-gray-600 font-medium">
										<div className="font-bold text-[#0B4D1E]">
											{formatTanggal(item.tanggal)}
										</div>
										<div className="text-xs text-gray-400">
											{item.nama_wilayah} (Oleh: {item.nama_petugas})
										</div>
									</td>
									...
									<td className="px-8 py-5 font-extrabold text-[#0B4D1E]">
										{(item.berat || item.berat_gram || 0) / 1000}
									</td>
									<td className="px-8 py-5 font-extrabold text-[#0B4D1E]">
										{formatRp(item.total_nilai)}
									</td>
									<td className="px-8 py-5">
										<span className="text-green-600 bg-green-100 px-3 py-1.5 rounded-full font-bold text-xs">
											{item.status}
										</span>
									</td>
						...
						<div className="space-y-6">
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								...
								<p className="font-extrabold text-[#0B4D1E]">
									{formatTanggal(selectedTrx.tanggal)}
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								<p className="text-gray-500 font-medium text-sm">Wilayah</p>
								<p className="font-extrabold text-[#0B4D1E]">
									{selectedTrx.nama_wilayah}
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								<p className="text-gray-500 font-medium text-sm">Kategori</p>
								<span className="bg-[#E8F5E9] text-[#2E7D32] px-4 py-1.5 rounded-full font-bold text-xs">
									{selectedTrx.nama_kategori}
								</span>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								<p className="text-gray-500 font-medium text-sm">Berat</p>
								<p className="font-extrabold text-[#0B4D1E] text-lg">
									{(selectedTrx.berat || selectedTrx.berat_gram || 0) / 1000} kg
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								...
								<p className="font-extrabold text-green-600 text-lg">
									{formatRp(selectedTrx.total_nilai)}
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
								<p className="text-gray-500 font-medium text-sm">Petugas</p>
								<p className="font-extrabold text-[#0B4D1E]">
									{selectedTrx.nama_petugas}
								</p>
							</div>
							<div className="flex justify-between items-center border-b border-gray-100 pb-4">
