...
function DuiLeaderboardPage() {
  ...
  const periodeList = [
    ...
    { label: 'Semua Periode', start: 'all', end: 'all' } // Tambahin ini biar konsisten sama page lain
  ];
  ...
  // FETCH SEMUA DATA SEKALI SAJA DARI BACKEND
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        // Tarik semua data riwayat dari endpoint Nanda tanpa filter tanggal di URL
        ...
        // PROTEKSI 1: Pastikan data benar-benar array
        ...
        setRawData([]); // PROTEKSI 2: Kalo error, pastikan state tetap array kosong
      }
    };
    fetchLeaderboard();
  }, []);

  // LOGIKA FRONTEND FILTERING (Anti-Crash)
  useEffect(() => {
    if (rawData.length === 0) {
      setLeaderboardData([]);
      return;
    ...
    if (filterPeriode.start !== 'all') {
      const startDate = new Date(filterPeriode.start);
      const endDate = new Date(filterPeriode.end);
      
      filteredRaw = rawData.filter(item => {
        if (!item.tanggal_update && !item.tanggal) return false;
        const itemDate = new Date(item.tanggal_update || item.tanggal);
        return itemDate >= startDate && itemDate <= endDate;
      });
    }

    const sortedFiltered = [...filteredRaw].sort((a, b) => (b.poin_kpi || 0) - (a.poin_kpi || 0));

    const mappedData = sortedFiltered.map((item, index) => ({
      rank: index + 1,
      wilayah: item.nama_wilayah || '-',
      kpi: item.poin_kpi ?? 0,
      input: `${(item.total_berat_gram || 0) / 1000} kg`,
    ...
    <DuiLayout>
          ...
          {isMonthPickerOpen && (
            <div className="absolute top-full mt-3 right-0 w-80 bg-white p-4 rounded-[2rem] shadow-2xl border border-gray-100 z-50 flex flex-col gap-2">
              <div className="grid grid-cols-2 gap-2">
                {periodeList.filter(p => p.label !== 'Semua Periode').map((prd) => (
                  <button 
                    key={prd.label} 
              ...
              <button 
                onClick={() => { 
                  const semuaOpt = periodeList.find(p => p.label === 'Semua Periode');
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 mb-8">
        ...
        <div className="flex flex-col md:flex-row justify-center items-end gap-4 h-72 px-4 mb-10">
          ...
          {/* PROTEKSI 3: Optional Chaining (?.) di Top 3 */}
          <div className="w-full md:w-1/3 bg-[#E2E8F0] rounded-t-3xl h-[75%] flex flex-col items-center justify-center text-[#0B4D1E] relative shadow-lg hover:-translate-y-2 transition-transform cursor-pointer">
            ...
            <p className="font-bold text-lg text-center px-2">{leaderboardData[1]?.wilayah || 'Belum Ada Data'}</p>
          ...
          <div className="w-full md:w-1/3 bg-[#F4A300] rounded-t-3xl h-full flex flex-col items-center justify-center text-white relative shadow-2xl z-10 hover:-translate-y-3 transition-transform cursor-pointer">
            ...
            <p className="font-bold text-xl text-center px-2">{leaderboardData[0]?.wilayah || 'Belum Ada Data'}</p>
          ...
          <div className="w-full md:w-1/3 bg-[#CD7F32] rounded-t-3xl h-[60%] flex flex-col items-center justify-center text-white relative shadow-lg hover:-translate-y-2 transition-transform cursor-pointer">
            ...
            <p className="font-bold text-lg text-center px-2">{leaderboardData[2]?.wilayah || 'Belum Ada Data'}</p>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {visibleData.map((w, index) => (
                <tr key={w.rank || index} className="hover:bg-gray-50 transition-colors">
                  <td className="px-8 py-5">
                    <div className="w-8 h-8 bg-[#F5EFE6] rounded-full flex items-center justify-center font-extrabold text-[#0B4D1E]">{w.rank || index + 1}</div>
                  </td>
                  <td className="px-8 py-5 font-extrabold text-[#0B4D1E]">{w.wilayah || '-'}</td>
                  <td className="px-8 py-5 font-extrabold text-[#F4A300]">{w.kpi ?? '-'}</td>
                  <td className="px-8 py-5 font-bold text-[#0B4D1E]">{w.input || '0 kg'}</td>
                  <td className="px-8 py-5 font-extrabold text-green-600">{w.nilai || 'Rp 0'}</td>
              ...
              {/* PROTEKSI 4: Tampilkan pesan kalo data beneran kosong */}
              {leaderboardData.length === 0 && (
                <tr>
                  <td colSpan="6" className="text-center py-10 text-gray-400 font-medium italic">
                    Belum ada data KPI wilayah pada periode ini.
                  </td>
                </tr>
              )}
            </tbody>
