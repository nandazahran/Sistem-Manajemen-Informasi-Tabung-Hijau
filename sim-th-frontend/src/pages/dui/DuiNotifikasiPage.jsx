    ...
    <DuiLayout>
      ...
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Cari judul pesan atau deskripsi notifikasi..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#F5EFE6] px-14 py-4 rounded-2xl font-medium text-[#0B4D1E] outline-none focus:ring-2 focus:ring-[#0B4D1E]" />
        </div>
        <div className="relative w-48">
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full bg-[#F5EFE6] text-[#0B4D1E] font-bold px-5 py-4 rounded-2xl outline-none cursor-pointer appearance-none">
            <option>Semua</option>
            ...
            filteredNotifications.map((notif) => (
              <div key={notif.id} className={`flex justify-between items-center p-6 rounded-3xl transition-all cursor-pointer ${notif.isRead ? 'bg-white border border-gray-100 hover:shadow-sm' : 'bg-[#FDF6EA] border border-[#F4A300]/20'}`}>
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <p className="font-extrabold text-[#0B4D1E] text-base">{notif.judul}</p>
                    <p className="text-sm text-gray-500 font-medium mt-1">{notif.deskripsi || notif.pesan}</p>
                    <p className="text-[11px] text-gray-400 font-bold mt-2">{formatWaktuDB(notif.waktu)}</p>
                  </div>
                </div>
                {!notif.isRead && <div className="w-2.5 h-2.5 bg-[#F4A300] rounded-full mr-2"></div>}
              </div>
            ))
          ) : (
